...
func main() {
	flag.Parse()
	host := flag.Arg(0) // get the goodies
	if len(host) == 0 {
		fmt.Println("Incorrect Input, You Need To Specify A Host")
		os.Exit(1)
	}
	...
	fmt.Printf("Sending ICMP Request To %v (%v)\n", host, ipAddr)
...
func ping(ipAddr *net.IPAddr, icmpSeq int, reply chan string, timeout chan string) {
	...
	case ipv4.ICMPTypeDestinationUnreachable:
		reply <- fmt.Sprintf("%v is currently unreachable", ipAddr)
	default:
